...
public class Analyzer {
  ...
  public static void main( String argv[] ) throws IOException {
    ...
    op.setActiveModules(false, false, false, true,
                               false, false, true,
  ...
  private static void printResults( ListSentence ls, String format ) {
    ...
    else
    {
      ...
      while (sIt.hasNext()) {
        ...
        while (wIt.hasNext()) {
          Word w = wIt.next();
          Integer aN = w.getNAnalysis();
          ListAnalysis aL = w.getAnalysis();
          if(aN < 3) {
            Analysis aF = aL.front();
            Analysis aB = aL.back();
            Double probF = aF.getProb();
            Double probB = aB.getProb();
            System.out.print(aB.getLemma() + " " + probB.toString() + " " + aB.getTag() + " " + aF.getLemma() + " " + probF.toString() + " " + aF.getTag() + "\n");
          } else {
            while(aL.size() > 0) {
              Analysis aF = aL.front();
              Double probF = aF.getProb();
              System.out.print(aF.getLemma() + " " + probF.toString() + " " + aF.getTag() + " ");
              aL.popFront();
            }
            System.out.println();
          }
